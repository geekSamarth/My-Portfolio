...
const Header = () => {
  return (
    <header className="bg-[#010B13] fixed top-0 min-w-full py-4 border-b border-[rgba(255,255,255,0.3)] md:px-4 flex items-center justify-between text-white z-20">
      ...
      <div className="hidden md:block md:mr-3 lg:mr-10">
        <ul className="flex items-center gap-10">
          {navLinks.map((link) => {
            return (
              <NavLink key={link.id} hrefLang={link.path}>
                <li>{link.title}</li>
              </NavLink>
            );
          })}
